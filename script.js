    ...
    for (var obj1 in objArray) {
        for (var obj2 in walls) {
            if ((distanceNextFrame2(objArray[obj1], walls[obj2], radius) == true)) {
                var theta1 = objArray[obj1].angle();
                var phi = Math.atan2(walls[obj2].y1 - objArray[obj1].y, walls[obj2].x1 - objArray[obj1].x);
                var m1 = objArray[obj1].mass;
                var v1 = objArray[obj1].speed();

                var dx1F = (v1 * Math.cos(theta1 - phi) * (m1/2)) / (2*m1) * Math.cos(phi) + v1*Math.sin(theta1-phi) * Math.cos(phi+Math.PI/2);
                var dy1F = (v1 * Math.cos(theta1 - phi) * (m1/2)) / (2*m1) * Math.sin(phi) + v1*Math.sin(theta1-phi) * Math.sin(phi+Math.PI/2);
